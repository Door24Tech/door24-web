...
| --- | --- | --- |
| `prototype/v1/users/{uid}/mentor_assignment` | Stores each user’s mentor selection from quiz step 8. Fields: `mentorId` (`'astra' | 'luna' | 'poco'`), `assignedAt` (timestamp). | Mobile app only (rule: `isSelf(uid)`). |
| `prototype/v1/mentors/config/default` | Global settings consumed by the client. Fields: `activeVariant`, `anonNickname`. | Prototype admins. |
| `prototype/v1/mentors/variants/{variantId}` | Draft/published copies of mentor header content. See field table below. | Prototype admins. |
...
- Writes:
  - `prototype/v1/users/{uid}/mentor_assignment`: only that user (set by the mobile app during onboarding).
...
- `prototype/v1/mentors/*` — Admin-managed content. **Write access requires the `prototypeAdmin` custom claim.**
...
| --- | --- | --- | --- |
| `mentors/config/default` | `activeVariant` | string | Must match a document ID under `mentors/variants`. |
|  | `anonNickname` | string | Used when the mobile client cannot determine the user’s display name (anonymous users). |
| `mentors/variants/{variantId}` | `mentorRoleLabel` | `{ Astra, Luna, Poco }` | All keys required; strings shown beneath avatar (hidden in current UI but stored). |
...
1. **Create/Edit Variant**  
   - Add or edit a document under `prototype/v1/mentors/variants/{variantId}`.  
...
3. **Publish Variant**  
   ...
   - Update `prototype/v1/mentors/config/default.activeVariant` to the variant’s document ID.  
...
- The app attempts to load:
  ...
  2. `prototype/v1/mentors/config/default` → resolves `activeVariant` + `anonNickname`.
  3. `prototype/v1/mentors/variants/{activeVariant}` → resolves per-mentor fields.
...
- Writes under `prototype/v1/mentors/**`: only users with `prototypeAdmin` custom claim.
- Writes under `prototype/v1/users/{uid}/mentor_assignment/**`: only the signed-in user (`uid`). Admin tooling should not modify these documents.
